<template>
  <!-- Статистика -->
  <div class="s-stats">
    <SButton outlined color="red" @click="resetAllRatings">
      Сбросить рейтинги
    </SButton>
  ...
  <SButton @click="openForm" color="green">Добавить</SButton>

  <Dialog :isVisible="isVisible" @cancel="cancelForm" :title="editId ? 'Редактирование книги' : 'Добавление книги'">
    <!-- Форма -->
    <BookForm
        v-model="form"
        @cancel="cancelForm"
...
const saveBook = () => {
  ...
  const genre = Array.isArray(form.value.genre)
      ? form.value.genre
      : form.value.genre
          ? [form.value.genre]  // если строка, превращаем в массив
          : []                  // если null/undefined, создаём пустой массив


  if (editId.value) {
    const book = books.value.find(b => b.id === editId.value)
    Object.assign(book, {...form.value, genre})
  } else {
